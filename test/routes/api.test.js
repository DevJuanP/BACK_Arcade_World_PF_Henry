...
describe('Prueba de la ruta de cart/purchased', () => {
...
describe('Compra agregada a la base de datos', () => {
  it('compra agregada a la base de datos', async () => {
    const games = await Videogame.findAll();
    const gamesID = games.map(game => game.id);
    const randomid = gamesID[Math.floor(Math.random() * gamesID.length)];
...
describe("Probando las rutas de videojuegos", () => {
  it('La respuesta de la API debe contener elementos en el array con 100 juegos', async () => {
    const response = await axios.get(`${serverUrl}/videogame`);
  ...
  it('La respuesta de la API debe traer los juegos por nombre', async () => {
    ...
    // Verifica que la longitud del array sea la esperada.
...
describe("Test de ruta de usuario", () => {
  ...
  it('Test de update de usuario', async () => {
    // Realiza una prueba de actualización de usuario
    // Asegúrate de proporcionar datos válidos para la actualización
    const requestBody = {
      // Proporciona los datos de actualización aquí
    };
  ...
  it('La respuesta de la API debe traer un usuario por ID', async () => {
    ...
    // Verifica que la respuesta contiene las propiedades que esperas
    // en la respuesta del usuario por ID.
  });
});
